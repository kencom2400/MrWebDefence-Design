...
- [ ] FQDN管理機能が正常に動作する
- [ ] ログ閲覧機能が正常に動作する
- [ ] WAFエンジン向け設定配信APIが正常に動作する
- [ ] WAFエンジン向けAPIトークン管理機能が正常に動作する
...
**なぜやるか**
WAFエンジン（設定取得エージェント）が設定を取得するためのAPIが必要。
...
**なぜやるか**
WAFエンジン（設定取得エージェント）が管理APIにアクセスするためのAPIトークンを生成・管理する機能が必要。
...
**受け入れ条件**
- [ ] APIトークン生成・発行機能が正常に動作する
- [ ] APIトークン一覧表示機能が正常に動作する
- [ ] APIトークン有効期限管理が正常に動作する（90日）
- [ ] APIトークンローテーション機能が正常に動作する
- [ ] APIトークン削除・無効化機能が正常に動作する
- [ ] APIトークン管理画面UIが完成している
...
- GeoIP機能実装
- ヘルスチェック機能実装
- インストール・セットアップスクリプト実装
- SSL/TLS証明書管理機能実装

#### 受け入れ条件
- [ ] Docker Compose構成が正常に動作する
...
- [ ] GeoIP機能が正常に動作する
- [ ] ヘルスチェック機能が正常に動作する
- [ ] インストール・セットアップスクリプトが正常に動作する
- [ ] SSL/TLS証明書管理機能が正常に動作する
...
- API呼び出し失敗時のリトライロジック実装（指数バックオフ）
- OpenAppSec設定ファイル生成機能実装
- Nginx設定ファイル生成機能実装（FQDN別）
- 無効化されたFQDNの設定ファイル削除機能実装
- 設定ファイルリロード機能実装（再起動不要）
- 設定のローカルキャッシュ機能実装（TTL: 5分）
- 設定取得エージェント起動スクリプト実装（`scripts/openappsec/start-config-agent.sh`）

**受け入れ条件**
- [ ] 管理APIから設定取得が正常に動作する
- [ ] API呼び出し失敗時のリトライロジックが正常に動作する
- [ ] OpenAppSec設定ファイルが正常に生成される
- [ ] Nginx設定ファイル（FQDN別）が正常に生成される
- [ ] 無効化されたFQDNの設定ファイルが正常に削除される
...
- ログローテーション設定実装（logrotate）
  - logrotate設定ファイルの作成
  - Nginxアクセスログのローテーション設定（JSON形式）
...
- [ ] エラーログが正常に転送される
- [ ] ログ転送のリトライ機能が正常に動作する
- [ ] logrotate設定ファイルが完成している
- [ ] Nginxアクセスログのローテーションが正常に動作する
- [ ] Nginxエラーログのローテーションが正常に動作する
- [ ] OpenAppSecログのローテーションが正常に動作する
...
- [ ] 設定取得エージェントの状態チェックが正常に動作する
- [ ] Redis接続チェックが正常に動作する
- [ ] 設定ファイルの存在確認が正常に動作する
- [ ] ヘルスチェックレスポンスがJSON形式で返却される
- [ ] ヘルスチェックスクリプトが正常に動作する
...
**なぜやるか**
各FQDNごとにSSL/TLS証明書を設定し、自動更新する機能が必要。
...
**受け入れ条件**
- [ ] 各FQDNごとのSSL/TLS証明書設定が正常に動作する
- [ ] Let's Encrypt証明書の自動取得が正常に動作する
- [ ] Let's Encrypt証明書の自動更新が正常に動作する
- [ ] 独自証明書の設定機能が正常に動作する
- [ ] 証明書の有効期限監視が正常に動作する
- [ ] 証明書の自動ローテーションが正常に動作する
- [ ] Nginx設定への証明書パス設定が正常に動作する
