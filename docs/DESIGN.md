...
**管理リポジトリ**:
- **MrWebDefence-Design**（このリポジトリ）でDB関連リソースを一元管理します
  - データベース初期化スクリプト（`scripts/database/`）
  - Flywayマイグレーションファイル（将来実装予定）
  - データベース設計ドキュメント（`docs/DESIGN.md`）

**配信先リポジトリ**:
- **MrWebDefence-Console**: 管理画面（フロントエンド・バックエンド）- DBに接続する主要アプリケーション
- その他のDB接続アプリケーション（将来追加される可能性がある）

**管理方針の理由**:
1. **設計との整合性**: データベース設計ドキュメントとDB関連リソースを同じリポジトリで管理することで、整合性が保たれる
2. **Single Source of Truth**: 設計リポジトリがDB関連リソースの唯一の情報源として機能する
3. **配信の柔軟性**: DBに接続するアプリケーションリポジトリに必要なリソースを配信できる
4. **保守性**: DB関連の変更を一箇所で管理でき、変更の影響範囲が明確になる

**配信方法（将来実装予定）**:

以下のいずれかの方法で、DB関連リソースをアプリケーションリポジトリに配信します：

1. **Gitサブモジュール**（推奨）
   - メリット: バージョン管理が明確、自動更新可能
   - デメリット: サブモジュールの管理が必要
   - 用途: 本番環境での使用

2. **CI/CDでの自動コピー**
   - メリット: シンプル、自動化可能
   - デメリット: コピー元とコピー先の同期が必要
   - 用途: 開発環境・CI環境での使用

3. **パッケージ化（npmパッケージなど）**
   - メリット: バージョン管理が明確、依存関係が明確
   - デメリット: パッケージの公開・管理が必要
   - 用途: 複数のアプリケーションで共有する場合
...
**概要**:
DB関連リソースのリポジトリ管理方針を段階的に実装します。初期段階では手動での配信から開始し、段階的に自動化を進めます。

**実装フェーズ**:

##### フェーズ1: 基盤整備（短期：Task 2.4完了後）

**目標**: DB関連リソースの管理基盤を整備し、手動配信の仕組みを確立する
...
**検証方法**:
- MrWebDefence-Consoleリポジトリでスクリプトが正常に動作することを確認
- 開発環境でのデータベース初期化が正常に完了することを確認

##### フェーズ2: Flywayマイグレーションファイル実装（中期：Task 2.5以降）

**目標**: Flywayマイグレーションファイルを実装し、配信仕組みを確立する

**実装内容**:
1. **Flywayマイグレーションファイルの実装**
   - 全テーブル作成用マイグレーションファイルの作成
   - 初期データ投入用Repeatable Migrationsの作成
   - マイグレーションファイルの配置（`src/main/resources/db/migration/`、`src/main/resources/db/seed/`）

2. **配信方法の改善**
   - **Gitサブモジュール**の導入を検討・実装
   - サブモジュール管理手順の確立
   - バージョン管理方針の明確化
...
**成果物**:
- Flywayマイグレーションファイル一式
- Gitサブモジュール設定
- サブモジュール管理手順ドキュメント
- CI/CD統合設計書

**検証方法**:
- Gitサブモジュール経由でMrWebDefence-Consoleに配信できることを確認
- マイグレーションが正常に実行されることを確認
- バージョン管理が適切に機能することを確認

##### フェーズ3: 自動配信の実装（長期：複数アプリケーション対応時）

**目標**: CI/CDでの自動配信を実装し、複数アプリケーションへの配信を効率化する

**実装内容**:
1. **CI/CD自動配信の実装**
   - GitHub Actions/GitLab CIでの自動配信パイプライン実装
   - 配信先リポジトリへの自動コピー機能
   - バージョンタグに基づく配信制御
...
- 複数アプリケーションへの配信が正常に完了することを確認
- 監視・通知機能が正常に動作することを確認

**配信方法の選定基準**:

| 配信方法 | 適用フェーズ | 選定条件 |
|---------|------------|---------|
| 手動コピー | フェーズ1 | 初期段階、単一アプリケーション、頻繁な変更が必要な場合 |
| Gitサブモジュール | フェーズ2 | バージョン管理が必要、本番環境での使用、複数アプリケーション対応 |
| CI/CD自動コピー | フェーズ3 | 自動化が必要、複数アプリケーション対応、頻繁な更新がある場合 |
| パッケージ化 | フェーズ3 | 複数アプリケーションで共有、依存関係管理が必要な場合 |
| シンボリックリンク | 開発環境のみ | ローカル開発環境、即座に反映が必要な場合 |

**実装手順（フェーズ1）**:

1. **MrWebDefence-Designでのリソース準備**
   ```bash
   # リソースの配置確認
   ls -la scripts/database/
   ```

2. **MrWebDefence-Consoleへの手動配信**
   ```bash
   # MrWebDefence-Consoleリポジトリに移動
   cd ../MrWebDefence-Console
   
   # リソースをコピー
   cp -r ../MrWebDefence-Design/scripts/database/ scripts/
...
3. **動作確認**
   - スクリプトが正常に実行できることを確認
   - データベース初期化が正常に完了することを確認

**実装手順（フェーズ2 - Gitサブモジュール）**:

1. **MrWebDefence-Designでサブモジュール用ブランチ/タグの作成**
   ```bash
   # リリース用タグの作成
   git tag -a v1.0.0-db-resources -m "Initial DB resources release"
   git push origin v1.0.0-db-resources
   ```

2. **MrWebDefence-Consoleでサブモジュールの追加**
   ```bash
   # サブモジュールの追加
   git submodule add https://github.com/kencom2400/MrWebDefence-Design.git db-resources
   cd db-resources
   git checkout v1.0.0-db-resources
   cd ..
...
3. **サブモジュールの更新**
   ```bash
   # サブモジュールの更新
   git submodule update --remote db-resources
   ```
...
| リスク | 影響度 | 対策 |
|--------|--------|------|
| 配信先リポジトリとの同期が取れない | 高 | バージョン管理を明確化、定期的な同期チェック |
| サブモジュールの管理が複雑になる | 中 | サブモジュール管理手順の明確化、自動化ツールの導入 |
| CI/CD自動配信の失敗 | 中 | エラーハンドリングの実装、監視・通知機能の実装 |
| 複数アプリケーションでのバージョン不整合 | 高 | バージョン管理方針の明確化、互換性チェックの実装 |

**タイムライン（目安）**:

- **フェーズ1**: Task 2.4完了後、1-2週間
- **フェーズ2**: Task 2.5以降、2-4週間
- **フェーズ3**: 複数アプリケーション対応時、4-8週間

**次のステップ**:

1. フェーズ1の実装完了後、MrWebDefence-Consoleリポジトリでの動作確認
2. Flywayマイグレーションファイルの実装開始（Task 2.5以降）
3. Gitサブモジュールの導入検討（フェーズ2）
4. CI/CD自動配信の要件定義（フェーズ3準備）
