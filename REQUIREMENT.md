...
- ログ管理サーバ
  - WAFエンジンから転送されるログを取り扱うことができる。小規模であればローカルに保管し、規模が大きい場合は他のログ管理機能、S3等のストレージに転送する機能を持つ
...
- シグニチャ収集機能
  ...
  - 新規に作成したシグニチャは、過去のWEBサーバ、WAFログを検査し、投入すべきかどうかを確認できる。これは毎日決まった時間に確認を行うバッチが稼働する。
  - また１回投入すべきではないと判断したシグニチャも30日間は保管し、毎日検査を行う
  - 検査の結果、WAF全体のシグニチャに投入すべきであると判断したものは、投入シグニチャ候補プールに投入をする。これはサービス全体管理画面の投入シグニチャ候補画面で確認できる
