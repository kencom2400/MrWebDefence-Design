...
|--------|--------|-----------|-----------|---------|------|
| ユーザー（ブラウザ） | 管理画面 | HTTPS | 443 | なし（TLS証明書による暗号化） | 外部アクセス |
...
| 設定管理API | 設定管理サービス | HTTP | 8080 | なし（内部通信） | 同一ネットワーク内、認証不要 |
| ユーザー管理API | ユーザー管理サービス | HTTP | 8081 | なし（内部通信） | 同一ネットワーク内、認証不要 |
| ログ取得API | ログ管理サービス | HTTP | 8082 | なし（内部通信） | 同一ネットワーク内、認証不要 |
